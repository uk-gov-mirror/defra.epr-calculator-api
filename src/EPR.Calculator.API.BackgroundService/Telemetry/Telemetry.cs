...
/// <summary>
///     An <see cref="ITelemetry" /> whose activities and metrics are categorised under <typeparamref name="TCategory" />'s
...
{
    public const string Name = "epr.paycal";

    /// <summary>
    ///     Activity tag carrying the logging category for the type that started the activity (see
    ///     <see cref="Telemetry{TCategory}" />), so <see cref="Helpers.TelemetryLoggingHost" /> can log under a
    ///     per-type category instead of one fixed category for every activity.
    /// </summary>
    public const string CategoryTag = "category";

    public const string DurationThresholdTag = "duration_threshold";
    protected static readonly TimeSpan DefaultDurationThreshold = TimeSpan.FromSeconds(10);

    public static readonly ActivitySource ActivitySource = new(Name);
    public static readonly Meter Meter = new(Name);

    /// <summary>
    ///     The ambient scope established by <see cref="BeginScopeCore" />. Backed by an
    ///     <see cref="AsyncLocal{T}" /> so it flows across async calls, and declared on this
    ///     non-generic base - rather than on <see cref="Telemetry{TCategory}" /> - so it is shared across every
...
{
    /// <summary>
    ///     A shared instance for <typeparamref name="TCategory" />, used by <see cref="ActivityMetricAttribute" />
    ///     when the target type has no <see cref="ITelemetry" /> constructor parameter.
    /// </summary>
    ...
    private static Activity? StartActivity(string name, TimeSpan? warningThreshold = null)
    {
        ...
        [
            ...
            new (DurationThresholdTag, warningThreshold ?? DefaultDurationThreshold),
            .. AmbientScope.Value?.ActivityTags ?? []
        ];

        return ActivitySource.StartActivity(name, ActivityKind.Internal, default(ActivityContext), tags);
    }
}
