...
public sealed partial class TelemetryLoggingHost(ILoggerFactory loggerFactory) : IHostedService, IDisposable
{
    ...
    {
        activityListener.ShouldListenTo = source => source.Name == Telemetry.Name;
        ...
        activityListener.ActivityStarted = activity =>
        {
            ...
            LogActivityStarting(activityLogger, activity.DisplayName);
        };
        ...
        activityListener.ActivityStopped = activity =>
        {
            ...
            var threshold = activity.GetTagItem(Telemetry.DurationThresholdTag) as TimeSpan?;

            if(threshold > TimeSpan.Zero && activity.Duration > threshold)
                LogWarningActivityEnded(activityLogger, activity.DisplayName, state, activity.Duration, threshold.Value);
            else
                LogActivityEnded(activityLogger, activity.DisplayName, state, activity.Duration);
        };
