...
) : IDataLoader
{
    ...
    public async Task LoadData(CalculatorRunContext runContext, CancellationToken cancellationToken = default)
    {
        ...
        await Run(runContext, timeProvider.GetUtcNow(), cancellationToken);
    }

    private async Task Run(RunContext runContext, DateTimeOffset loadTime, CancellationToken cancellationToken)
    {
